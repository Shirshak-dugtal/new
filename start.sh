...
if not User.objects.filter(username='student').exists():
    User.objects.create_user(username='student', email='student@test.com', password='test123', role='user')
    print('✅ Student account created (student/test123)')
else:
    print('ℹ️  Student account already exists')

if not User.objects.filter(username='instructor').exists():
    User.objects.create_user(username='instructor', email='instructor@test.com', password='test123', role='creator')
    print('✅ Instructor account created (instructor/test123)')
else:
    print('ℹ️  Instructor account already exists')
"
